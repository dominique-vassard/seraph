defmodule Seraph.Query.Builder.OnMatchSetTest do
  use ExUnit.Case, async: true

  alias Seraph.Query.Builder.OnMatchSet

  describe "build/2" do
    test "u.firstName = \"Jack\"" do
      ast = quote do: [u.firstName = "Jack"]

      %{on_match_set: %OnMatchSet{expressions: expressions}, params: params} =
        OnMatchSet.build(ast, __ENV__)
    ...
    test "u.lastName = ^ln" do
      ...
      assert [ln: {:ln, [], Seraph.Query.Builder.OnMatchSetTest}] == params
    end
  ...
  describe "check/2" do
    test "ok" do
      ...
      %{on_match_set: on_match_set, params: params} = OnMatchSet.build(ast, __ENV__)

      assert :ok = OnMatchSet.check(on_match_set, query_fixtures(params))
    end
    ...
    test "fails: Property - unknown entity identifier" do
      ...
      assert {:error, message} = OnMatchSet.check(on_match_set, query_fixtures(params))
      assert String.starts_with?(message, "[OnMatchSet] Entity with identifier")
    end
